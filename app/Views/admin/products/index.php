		...
		<div class="row">
		  	<div class="col-12">
				<div class="card">
					...
					<div class="card-tools">
						<form action="<?= site_url('admin/products') ?>" method="get" class="form-inline">
			  ...
			  <!-- /.card-header -->
					...
					<tbody>
						<?php if ($products): ?>
							<?php foreach ($products as $product): ?>
								<tr>
									<td><?= $product->sku ?></td>
									<td><?= !empty($product->featured_image) ? '<img src="' . base_url($product->featured_image->small) . '">' : null ?></td>
									<td>
										<?= $product->name ?><br/>
									...
									<td><?= $product->price ?></td>
									<td><?= $product->qty ?></td>
									<td><?= ($product->published_at) ? 'Display in Catalog' : 'Save as Draft' ?></td>
									<td>
										<?php if (empty($product->deleted_at)): ?>
											<a href="<?= site_url('admin/products/edit/'. $product->id) ?>" class="badge bg-info">edit</a>
				...
				<div class="card-footer clearfix">
					...
					<div class="row">
						<div class="col-8">
							<?php echo $pager->links('bootstrap', 'bootstrap') ?>
