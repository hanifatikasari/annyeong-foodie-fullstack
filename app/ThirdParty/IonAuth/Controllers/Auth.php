...
class Auth extends \CodeIgniter\Controller
{
	 ...
	 */
	protected $validationListTemplate = 'auth\Messages\list';
	...
	public function index()
	{
		...
		else if (! $this->auth->isAdmin()) // remove this elseif if you want to enable this for non-admins
		{
	...
	public function login()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run())
		{
			...
			{
				// Gunakan setFlashdata agar pesan "nyangkut" saat di-redirect balik
		...
		else
		{
			...
			$this->data['identity'] = [
				...
				'class' => 'form-control text-dark', // Tambahkan ini
   				'placeholder' => 'Email atau Username', // Tambahkan ini biar lebih keren
			];
			...
			$this->data['password'] = [
				...
				'type' => 'password',
				'class' => 'form-control text-dark', // Tambahkan ini
   				'placeholder' => 'Password', // Tambahkan ini biar lebih keren
			];
	...
	public function forgot_password()
	{
		...
		else
		{
			...
			$identity = $this->auth->where($identityColumn, $this->request->getPost('identity'))->users()->row();
		...
		if ($user)
		{
			...
			else
			{
				...
				else
				{
					...
					$change = $this->auth->resetPassword($identity, $this->request->getPost('new'));
	...
	public function deactivate(int $id = 0)
	{
		...
		else
		{
			...
			if ($this->request->getPost('confirm') === 'yes')
			{
				...
				if ($this->auth->loggedIn() && $this->auth->isAdmin())
				{
					$message = $this->auth->deactivate($id) ? $this->auth->messages() : $this->auth->errors($this->validationListTemplate);
	...
	public function create_user()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run() && $this->auth->register($identity, $password, $email, $additionalData))
		{
	...
	public function edit_group(int $id = 0)
	{
		...
		if ($this->request->getPost())
		{
			if ($this->validation->withRequest($this->request)->run())
			{
				$groupUpdate = $this->auth->updateGroup($id, $this->request->getPost('group_name'), ['description' => $this->request->getPost('group_description')]);
		...
		// set the flash data error message if there is one
		$this->data['message'] = $this->validation->listErrors($this->validationListTemplate) ?: ($this->auth->errors($this->validationListTemplate) ?: $this->session->getFlashdata('message'));
